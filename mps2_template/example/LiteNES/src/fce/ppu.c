...
NO_INIT byte ppu_sprite_palette[4][4];
NO_INIT bool ppu_2007_first_read;
NO_INIT byte ppu_addr_latch;
NO_INIT byte ppu_latch;

NO_INIT byte PPU_SPRRAM[0x100];
NO_INIT byte PPU_RAM[0x4000];

NO_INIT byte ppu_screen_background[264][248];

// Precalculated tile high and low bytes addition for pattern tables
NO_INIT byte ppu_l_h_addition_table[256][256][8];
NO_INIT byte ppu_l_h_addition_flip_table[256][256][8];
...
static const word ppu_base_nametable_addresses[] = { 0x2000, 0x2400, 0x2800, 0x2C00 };

inline word ppu_base_nametable_address(void)                            { return ppu_base_nametable_addresses[ppu.PPUCTRL & 0x3];  }
inline byte ppu_vram_address_increment(void)                            { return common_bit_set(ppu.PPUCTRL, 2) ? 32 : 1;          }
inline word ppu_sprite_pattern_table_address(void)                      { return common_bit_set(ppu.PPUCTRL, 3) ? 0x1000 : 0x0000; }
inline word ppu_background_pattern_table_address(void)                  { return common_bit_set(ppu.PPUCTRL, 4) ? 0x1000 : 0x0000; }
inline byte ppu_sprite_height(void)                                     { return common_bit_set(ppu.PPUCTRL, 5) ? 16 : 8;          }
inline bool ppu_generates_nmi(void)                                     { return common_bit_set(ppu.PPUCTRL, 7);                   }



// PPUMASK Functions

inline bool ppu_renders_grayscale(void)                                 { return common_bit_set(ppu.PPUMASK, 0); }
inline bool ppu_shows_background_in_leftmost_8px(void)                  { return common_bit_set(ppu.PPUMASK, 1); }
inline bool ppu_shows_sprites_in_leftmost_8px(void)                     { return common_bit_set(ppu.PPUMASK, 2); }
inline bool ppu_shows_background(void)                                  { return common_bit_set(ppu.PPUMASK, 3); }
inline bool ppu_shows_sprites(void)                                     { return common_bit_set(ppu.PPUMASK, 4); }
inline bool ppu_intensifies_reds(void)                                  { return common_bit_set(ppu.PPUMASK, 5); }
inline bool ppu_intensifies_greens(void)                                { return common_bit_set(ppu.PPUMASK, 6); }
inline bool ppu_intensifies_blues(void)                                 { return common_bit_set(ppu.PPUMASK, 7); }
...
inline bool ppu_sprite_overflow(void)                                   { return common_bit_set(ppu.PPUSTATUS, 5); }
inline bool ppu_sprite_0_hit(void)                                      { return common_bit_set(ppu.PPUSTATUS, 6); }
inline bool ppu_in_vblank(void)                                         { return common_bit_set(ppu.PPUSTATUS, 7); }
...
inline uint_fast16_t ppu_get_real_ram_address(uint_fast16_t address)
{
    ...
    for (tile_x = ppu_shows_background_in_leftmost_8px() ? 0 : 1; tile_x < 32; tile_x++) {
        ...
        for (x = 0; x < 8; x++) {
            ...
            if (color != 0) {
                ...
                bool top = (ppu.scanline & (32 - 1)) < 16;
                bool left = ((tile_x & (32-1)) < 16);
                ...
                pixbuf_add(
                    bg, 
                    (tile_x << 3) + x - ppu.PPUSCROLL_X + (mirror ? 256 : 0), 
                    ppu.scanline + 1, 
                    idx);
            }
        }
...
void ppu_draw_sprite_scanline(void)
{
...
void ppu_cycle(void)
{
    ...
    if (ppu_shows_background()) {
        ...
        ppu_draw_background_scanline(true);         //! the possible mirror issue
    }
...
inline void ppu_copy(uint_fast16_t address, byte *source, int_fast32_t length)
{
    memcpy(&PPU_RAM[address], source, length);
}

inline uint_fast8_t ppu_io_read(uint_fast16_t address)
{
/*
    The PPU exposes eight memory-mapped registers to the CPU. These nominally 
    sit at $2000 through $2007 in the CPU's address space, but because they're 
    incompletely decoded, they're mirrored in every 8 bytes from $2008 through 
    $3FFF, so a write to $3456 is the same as a write to $2006.
 */
    ppu.PPUADDR &= 0x3FFF;
    switch (address & 7) {
        case 2:
...
inline void ppu_io_write(uint_fast16_t address, uint_fast8_t data)
{
/*
    This bus, called _io_db in Visual 2C02 and PPUGenLatch in FCEUX,[1] behaves 
    as an 8-bit dynamic latch due to capacitance of very long traces that run to 
    various parts of the PPU. Writing any value to any PPU port, even to the 
    nominally read-only PPUSTATUS, will fill this latch. Reading any readable 
    port (PPUSTATUS, OAMDATA, or PPUDATA) also fills the latch with the bits 
    read. Reading a nominally "write-only" register returns the latch's current 
    value, as do the unused bits of PPUSTATUS. This value begins to decay after 
    a frame or so, faster once the PPU has warmed up, and it is likely that 
    values with alternating bit patterns (such as $55 or $AA) will decay faster.
 */
    ppu_latch = data;
...
void ppu_sprram_write_block(const byte *pchSrc, uint_fast32_t wSize)
{
    memcpy(&PPU_SPRRAM[ppu.OAMADDR], pchSrc, wSize);
    ppu.OAMADDR += wSize;
}
/*
void ppu_sprram_write(uint_fast8_t data)
{
    PPU_SPRRAM[ppu.OAMADDR++] = data;
}
*/
